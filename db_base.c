...
//	printf("num cols:%d \n", cols);
	...
	for (i = 0; i < cols; i++)
	{
	//	printf("%s(type:%d)\t", sel_field[i].name, sel_field[i].type); //打印表头信息和类型
	printf("%s\t", sel_field[i].name); //仅打印表头信息
	}
	printf("\n");

	while (sel_row = mysql_fetch_row(sel_res))   //打印每行结果
	{
		for (i = 0; i < cols; i++)
		{
			printf("%s\t", sel_row[i]);
